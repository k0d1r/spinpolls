const admin = require('firebase-admin');

// Service account ile bağlantı kur
const serviceAccount = {
  type: "service_account",
  project_id: "kadircloudcom",
  private_key_id: "015616820366bedc3800196ca6c5016d96f82323",
  private_key: "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
  client_email: "firebase-adminsdk-fbsvc@kadircloudcom.iam.gserviceaccount.com",
  client_id: "102780990422357668131",
  auth_uri: "https://accounts.google.com/o/oauth2/auth",
  token_uri: "https://oauth2.googleapis.com/token",
  auth_provider_x509_cert_url: "https://www.googleapis.com/oauth2/v1/certs",
  client_x509_cert_url: "https://www.googleapis.com/robot/v1/metadata/x509/firebase-adminsdk-fbsvc%40kadircloudcom.iam.gserviceaccount.com",
  universe_domain: "googleapis.com"
};

// Firebase Admin SDK'yı başlat
admin.initializeApp({
  credential: admin.credential.cert(serviceAccount),
  projectId: 'kadircloudcom'
});

const db = admin.firestore();

// Firestore index'leri oluştur
async function createFirestoreIndexes() {
  try {
    console.log('🔍 Firestore index\'leri oluşturuluyor...');
    
    // Wheels koleksiyonu için index
    const wheelsRef = db.collection('wheels');
    await wheelsRef.where('createdBy', '==', 'test-user').orderBy('createdAt', 'desc').limit(1).get();
    
    console.log('✅ Wheels index oluşturuldu');
    
    // Polls koleksiyonu için index
    const pollsRef = db.collection('polls');
    await pollsRef.where('createdBy', '==', 'test-user').orderBy('createdAt', 'desc').limit(1).get();
    
    console.log('✅ Polls index oluşturuldu');
    
    console.log('\n🎉 Tüm index\'ler oluşturuldu!');
    console.log('📋 Firebase Console\'da index\'leri kontrol edin:');
    console.log('1. https://console.firebase.google.com');
    console.log('2. kadircloudcom projesini seçin');
    console.log('3. Firestore Database → Indexes');
    
  } catch (error) {
    console.error('❌ Index oluşturma hatası:', error);
    console.log('\n🔧 Çözüm:');
    console.log('1. Firebase Console\'da manuel olarak index oluşturun');
    console.log('2. Hata mesajındaki linki kullanın');
  }
}

createFirestoreIndexes();
